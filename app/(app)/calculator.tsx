...
export default function Calculator() {
  ...
  const handleSave = async () => {
    ...
    const recordData: Partial<ClinicalRecord> = {
      ...
      bmi: results.bmi ?? undefined, bmi_status: results.bmiStatus ?? undefined,
      ideal_weight: results.idealWeight ?? undefined, fat_percent: results.fatPercent ?? undefined, 
      fat_mass_kg: results.fatMassKg ?? undefined, lean_mass_kg: results.leanMassKg ?? undefined, 
      ict: results.ict ?? undefined, cv_risk: results.cvRisk ?? undefined,
      systolic_bp: parseInt(form.systolic_bp || '0') || undefined, 
      diastolic_bp: parseInt(form.diastolic_bp || '0') || undefined,
      bp_status: results.bpStatus ?? undefined, activity_factor: val(form.activity_factor),
      bmr_kcal: Math.round(results.bmr || 0) || undefined, 
      tdee_kcal: Math.round(results.tdee || 0) || undefined, 
      water_liters: val(results.waterLiters),
      macro_prot_pct: parseInt(form.macro_prot_pct || '0') || undefined, 
      macro_cho_pct: parseInt(form.macro_cho_pct || '0') || undefined,
      macro_fat_pct: parseInt(form.macro_fat_pct || '0') || undefined, 
      ...
      ai_analysis: aiAnalysis?.summary || undefined, 
      grip_strength_kg: val(form.grip_strength_kg),
      calf_circumference_cm: val(form.calf_circumference_cm),
      somatotype: results.somatotype ?? undefined,
      
      // Additional fields from form/results
      creatinine: val(form.creatinine),
      gfr: results.gfr ?? undefined,
      kdigo_stage: results.kdigoStage ?? undefined,
      glucose_mg: parseInt(form.glucose_mg || '0') || undefined,
      hba1c: val(form.hba1c),
      total_chol: parseInt(form.total_chol || '0') || undefined,
      hdl: parseInt(form.hdl || '0') || undefined,
      ldl: parseInt(form.ldl || '0') || undefined,
      triglycerides: parseInt(form.triglycerides || '0') || undefined,
      hemoglobin: val(form.hemoglobin),
      ferritin: val(form.ferritin),
      albumin: val(form.albumin),
      heart_rate: parseInt(form.heart_rate || '0') || undefined,
      temperature: val(form.temperature),
      oxygen_sat: parseInt(form.oxygen_sat || '0') || undefined,
      ...
      weight_loss_weeks: parseInt(form.weight_loss_weeks || '0') || undefined,
    ...
    if (!session?.user) {
      ...
      Alert.alert(
        ...
        [
          { text: 'GENERAR PDF', onPress: () => generateClinicalReport(patient as any, form as any, results, (aiAnalysis || undefined) as any, []) },
          { text: 'ENTENDIDO' }
        ]
      );
      ...
      <View style={styles.container}>
        <View style={styles.tabContainer}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.tabScroll}>
            <TabItem id="centro" label="CENTRO" icon="apps" active={activeTab} onPress={() => router.push('/(app)/dashboard')} color={COLORS.orange} />
          ...
          {activeTab === 'scanner' && (
            <View style={styles.section}>
              ...
              <View style={styles.dashboardGrid}>
                ...
                <ResultBox label="RIESGO CV" value={results?.cvRisk || '--'} sub={`ICT: ${results?.ict?.toFixed(2) || '--'}`} color={results?.cvColor || COLORS.pink} />
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  tabContainer: { backgroundColor: 'rgba(5,5,10,0.9)', borderBottomWidth: 1, borderBottomColor: COLORS.crimson },
  tabScroll: { paddingHorizontal: 5 },
  tab: { paddingHorizontal: 12, paddingVertical: 14, flexDirection: 'row', alignItems: 'center', gap: 6, borderBottomWidth: 3, borderBottomColor: 'transparent' },
  tabText: { color: COLORS.muted, fontSize: 11, letterSpacing: 1, fontWeight: 'bold' },
  scrollContent: { padding: 16, paddingBottom: 80 },
  section: { gap: 20 },
  sectionHeader: { flexDirection: 'row', alignItems: 'center', gap: 10, borderLeftWidth: 5, paddingLeft: 12, backgroundColor: 'rgba(255,0,60,0.1)', paddingVertical: 10 },
  sectionTitle: { fontSize: 20, fontFamily: FONTS.horror, letterSpacing: 2, textShadowColor: '#000', textShadowRadius: 3 },
  row: { flexDirection: 'row', gap: 12 },
  grid2: { flexDirection: 'row', flexWrap: 'wrap', gap: 12 },
  inputGroup: { flex: 1, gap: 8 },
  inputLabel: { color: COLORS.bone, fontSize: 11, fontWeight: '900', letterSpacing: 1, textTransform: 'uppercase' },
  ...
  resBox: { width: '47%', backgroundColor: COLORS.bg2, padding: 16, borderBottomWidth: 4, gap: 6, shadowColor: '#000', shadowOffset: { width: 0, height: 5 }, shadowOpacity: 0.4, shadowRadius: 10, elevation: 6, borderRadius: 6 },
  resLabel: { fontSize: 10, fontWeight: '900', color: COLORS.muted, letterSpacing: 1 },
  resValue: { fontSize: 20, fontWeight: '900', fontFamily: FONTS.horror, textShadowColor: '#000', textShadowRadius: 3 },
  resSub: { fontSize: 10, color: COLORS.bone, fontWeight: '900', textTransform: 'uppercase' },
  
  mainActionBtn: { backgroundColor: COLORS.crimson, height: 60, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 12, marginTop: 30, shadowColor: COLORS.crimson, shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.6, shadowRadius: 15, elevation: 10, borderRadius: 8 },
  mainActionText: { color: COLORS.white, fontFamily: FONTS.horror, fontSize: 20, letterSpacing: 2 },
  
  macroDashboard: { flexDirection: 'column', gap: 12 },
  macroBox: { backgroundColor: COLORS.bg2, padding: 16, borderLeftWidth: 6, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', shadowColor: '#000', shadowOpacity: 0.3, shadowRadius: 6, borderRadius: 6 },
  macroLabel: { fontSize: 12, fontWeight: '900', color: COLORS.muted, letterSpacing: 1 },
  macroGrams: { fontSize: 24, fontWeight: '900', fontFamily: FONTS.horror },
  macroGkg: { fontSize: 12, color: COLORS.bone, fontWeight: 'bold' },
  
  utilityBox: { backgroundColor: 'rgba(255,255,255,0.03)', padding: 15, borderWidth: 1, borderColor: COLORS.dim, borderLeftWidth: 3, marginBottom: 10, flexDirection: 'row', gap: 10, alignItems: 'center', borderRadius: 4 },
  utilityText: { color: COLORS.muted, fontSize: 11, lineHeight: 16, flex: 1, fontWeight: '700' },
  sectionDesc: { color: COLORS.muted, fontSize: 12, marginBottom: 15, fontWeight: '800', letterSpacing: 0.5 },
  vizTitle: { color: COLORS.bone, fontSize: 12, fontWeight: '900', letterSpacing: 1, marginBottom: 15, textAlign: 'center', fontFamily: FONTS.horror },
  
  vizContainer: { alignItems: 'center', padding: 20, backgroundColor: 'rgba(255,255,255,0.04)', borderWidth: 2, borderColor: COLORS.purple, shadowColor: COLORS.purple, shadowOpacity: 0.2, shadowRadius: 10, borderRadius: 8 },
  miniCard: { backgroundColor: COLORS.bg2, padding: 20, borderWidth: 1.5, borderColor: COLORS.dim, gap: 15, borderRadius: 8 },
  cardTitle: { fontSize: 13, fontWeight: '900', color: COLORS.orange, letterSpacing: 1 },
  displayOnly: { flex: 1, justifyContent: 'center', backgroundColor: '#000', padding: 16, borderWidth: 1.5, borderColor: COLORS.purple, borderRadius: 6 },
  displayLabel: { fontSize: 10, fontWeight: '900', color: COLORS.muted, marginBottom: 6 },
  displayValue: { fontSize: 20, fontWeight: '900', color: COLORS.neon },
  
  dosageResult: { backgroundColor: 'rgba(255, 107, 0, 0.15)', padding: 20, alignItems: 'center', borderRadius: 6, borderWidth: 1.5, borderColor: COLORS.orange },
  dosageLabel: { fontSize: 11, color: COLORS.bone, fontWeight: '900', letterSpacing: 1.5 },
  dosageValue: { fontSize: 28, color: COLORS.orange, fontWeight: '900', marginTop: 10, fontFamily: FONTS.horror },
  
  aiCard: { backgroundColor: 'rgba(157, 0, 255, 0.05)', padding: 24, borderWidth: 2, borderColor: COLORS.purple, shadowColor: COLORS.purple, shadowOpacity: 0.3, shadowRadius: 15, borderRadius: 8 },
  aiLabel: { color: COLORS.crimson, fontSize: 14, fontWeight: '900', letterSpacing: 2, marginBottom: 10, fontFamily: FONTS.horror },
  aiText: { color: COLORS.white, fontSize: 14, lineHeight: 22, textAlign: 'justify', fontWeight: '800' },
  aiRecItem: { flexDirection: 'row', gap: 10, marginBottom: 12 },
  aiRecBullet: { color: COLORS.poison, fontWeight: '900', fontSize: 18 },
  aiDivider: { height: 2, backgroundColor: 'rgba(255,255,255,0.1)', marginVertical: 20 },
  
  actionRow: { flexDirection: 'row', gap: 15, marginTop: 25 },
  actionBtn: { height: 56, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, backgroundColor: COLORS.bg3, borderWidth: 2, borderColor: COLORS.purple, shadowColor: COLORS.purple, shadowOpacity: 0.3, shadowRadius: 10, borderRadius: 8 },
  actionBtnText: { color: COLORS.white, fontFamily: FONTS.horror, fontSize: 16 },
  
  identityCard: { backgroundColor: 'rgba(0, 255, 159, 0.03)', padding: 20, borderWidth: 2, borderColor: COLORS.dim, gap: 15, marginBottom: 15, borderRadius: 8 },
  miniBtnRow: { flexDirection: 'row', gap: 10 },
  miniBtn: { flex: 1, height: 48, borderWidth: 1.5, borderColor: COLORS.dim, justifyContent: 'center', alignItems: 'center', backgroundColor: '#000', borderRadius: 6 },
  activeMiniBtn: { borderColor: COLORS.crimson, backgroundColor: 'rgba(255, 0, 60, 0.2)' },
  miniBtnText: { color: COLORS.muted, fontSize: 14, fontWeight: '900' },
  activeMiniBtnText: { color: COLORS.white },
  
  searchBar: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#000', borderWidth: 2, borderColor: COLORS.gold, paddingHorizontal: 15, height: 50, gap: 12, marginBottom: 20, borderRadius: 6 },
  searchInput: { flex: 1, color: COLORS.white, fontSize: 14, fontWeight: '900', letterSpacing: 1 },
  iconBtn: { padding: 12, backgroundColor: COLORS.bg1, borderWidth: 1.5, borderColor: COLORS.dim, borderRadius: 6 },
  
  historyCard: { backgroundColor: COLORS.bg2, padding: 20, borderWidth: 1.5, borderColor: COLORS.dim, marginBottom: 15, borderLeftWidth: 6, borderLeftColor: COLORS.gold, shadowColor: '#000', shadowOpacity: 0.3, shadowRadius: 10, borderRadius: 6 },
  historyHead: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 15 },
  historyName: { fontSize: 18, color: COLORS.white, fontWeight: '900', letterSpacing: 1, marginBottom: 6 },
  historyDate: { fontSize: 12, color: COLORS.muted, fontWeight: 'bold' },
  historyGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 15 },
  hStat: { flex: 1 },
  hLabel: { fontSize: 10, color: COLORS.muted, fontWeight: '900', marginBottom: 4, letterSpacing: 0.5 },
  hVal: { fontSize: 16, fontWeight: '900' },
  emptyCard: { alignItems: 'center', justifyContent: 'center', padding: 40, borderWidth: 2, borderColor: COLORS.dim, borderStyle: 'dashed', borderRadius: 8 },
  emptyText: { color: COLORS.muted, fontFamily: FONTS.horror, fontSize: 20, marginTop: 15 },
  
  tableCard: { backgroundColor: COLORS.bg2, padding: 20, borderWidth: 1.5, borderColor: COLORS.dim, marginBottom: 15, borderRadius: 8 },
  tableTitle: { fontSize: 14, fontWeight: '900', color: COLORS.orange, marginBottom: 20, letterSpacing: 1.5, fontFamily: FONTS.horror },
  tableRow: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: COLORS.dim },
  tableLabel: { color: COLORS.bone, fontSize: 13, fontWeight: '900' },
  tableValue: { fontSize: 14, fontWeight: '900' },
  
  libTabs: { flexDirection: 'row', backgroundColor: '#000', borderWidth: 1.5, borderColor: COLORS.dim, marginBottom: 20, borderRadius: 6 },
  libTab: { flex: 1, paddingVertical: 14, alignItems: 'center', justifyContent: 'center', borderBottomWidth: 3, borderBottomColor: 'transparent' },
  activeLibTab: { borderBottomColor: COLORS.orange, backgroundColor: 'rgba(255, 107, 0, 0.1)' },
  libTabText: { color: COLORS.muted, fontSize: 10, fontWeight: '900', letterSpacing: 1 },
});
